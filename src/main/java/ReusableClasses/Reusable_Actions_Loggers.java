...
public class Reusable_Actions_Loggers {
    ...
    public static void actualTitle(WebDriver driver, String xpath, ExtentTest logger, String elementName) {
        WebDriverWait wait = new WebDriverWait(driver, timeout);
       try {
            WebElement element = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath)));
            element.submit();
            logger.log(LogStatus.PASS, "Successfully verified page Title " + elementName);
        } catch (Exception e) {
           System.out.println("Unable to verify the page Title " + elementName + " " + e);
           logger.log(LogStatus.FAIL, "Unable to verify the page Title " + elementName + " " + e);
       }
